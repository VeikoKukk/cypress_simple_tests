...
describe('This is first test suite, Veiko Kukk',() => {
    it('User can submit data only when valid mandatory values are added', () => {
    //Todays date is 27/04/2024
    cy.get('#lastName').type('Kukk')
    cy.get('#firstName').type('Veiko')
    ...
    */
    ...
    it('User cannot submit data when password and/or confirmation password is absent', () => {
        ...
        cy.get('#firstName').type('Veiko')
        cy.get('[data-testid="phoneNumberTestId"]').type('55666777')
            cy.get('input[name="password"]').clear('Parool123')
            cy.get('[name="confirm"]').clear('Parool123')
            cy.get('#username').type('Something')
            cy.get('h2').contains('Password').click()
        cy.get('.submit_button').should('be.disabled')
        cy.get('#success_message').should('not.be.visible')
    ...
    it('User cannot add letters to phone number', () => {
        ...
        // how we can check from html code, that phone number should contain only number
        cy.get('[data-testid="phoneNumberTestId"]').type('stghyj')
        cy.get('#username').type('johnDoe')
